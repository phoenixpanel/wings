...
* Support relative file paths for the Wings config ([#180](https://github.com/phoenixpanel/wings/pull/180))

### Fixed

* Folders not being sorted before files properly ([#5078](https://github.com/phoenixpanel/panel/issues/5078)

## v1.11.13

### Fixed

* Auto-configure not working ([#5087](https://github.com/phoenixpanel/panel/issues/5087))
* Individual files unable to be decompressed ([#5034](https://github.com/phoenixpanel/panel/issues/5034))

## v1.11.12

### Fixed
* Arbitrary File Write/Read ([GHSA-gqmf-jqgv-v8fw](https://github.com/phoenixpanel/wings/security/advisories/GHSA-gqmf-jqgv-v8fw))
* Server-side Request Forgery (SSRF) during remote file pull ([GHSA-qq22-jj8x-4wwv](https://github.com/phoenixpanel/wings/security/advisories/GHSA-qq22-jj8x-4wwv))
* Invalid `Content-Type` being used with the `wings diagnostics` command ([#186](https://github.com/phoenixpanel/wings/pull/186))

## v1.11.11
### Fixed
* Backups missing content when a `.pteroignore` file is used
* Archives originating from a subdirectory not containing any files ([#5030](https://github.com/phoenixpanel/panel/issues/5030))

## v1.11.10
### Fixed
* Archives randomly ignoring files and directories ([#5027](https://github.com/phoenixpanel/panel/issues/5027))
* Crash when deleting or transferring a server ([#5028](https://github.com/phoenixpanel/panel/issues/5028))
...
## v1.11.5
### Added
* Added a config option to disable Wings config.yml updates from the Panel (https://github.com/phoenixpanel/wings/commit/ec6d6d83ea3eb14995c24f001233e85b37ffb87b)

### Changed
* Wings is now built with Go 1.19.7

### Fixed
* Fixed archives containing partially matched file names (https://github.com/phoenixpanel/wings/commit/43b3496f0001cec231c80af1f9a9b3417d04e8d4)
...
* Adds support for configuring the MTU on the `phoenixpanel0` network.
...
### Added
* Added the ability to define the location of the temporary folder used by Wings — defaults to `/tmp/phoenixpanel`.
...
* Fixes files uploaded with SFTP not being owned by the phoenixpanel user.
...
* Fixes port bindings being incorrectly re-assigned when using `127.0.0.1` with Docker. These are now properly re-mapped to the `phoenixpanel0` interface so that networking operates as expected for the server.
...
## v1.0.0-beta.5
### Fixed
* Default config location settled on `/etc/phoenixpanel/config.yml`; wings will now check all of the previous locations for the configuration and move it automatically to the new location.
