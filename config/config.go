...
import (
	...
	"github.com/phoenixpanel/wings/system"
)

const DefaultLocation = "/etc/phoenixpanel/config.yml"
...
type SystemConfiguration struct {
	// The root directory where all of the phoenixpanel data is stored at.
	RootDirectory string `default:"/var/lib/phoenixpanel" json:"-" yaml:"root_directory"`

	// Directory where logs for server installations and other wings events are logged.
	LogDirectory string `default:"/var/log/phoenixpanel" json:"-" yaml:"log_directory"`

	// Directory where the server data is stored at.
	Data string `default:"/var/lib/phoenixpanel/volumes" json:"-" yaml:"data"`

	// Directory where server archives for transferring will be stored.
	ArchiveDirectory string `default:"/var/lib/phoenixpanel/archives" json:"-" yaml:"archive_directory"`

	// Directory where local backups will be stored on the machine.
	BackupDirectory string `default:"/var/lib/phoenixpanel/backups" json:"-" yaml:"backup_directory"`

	// TmpDirectory specifies where temporary files for phoenixpanel installation processes
	// should be created. This supports environments running docker-in-docker.
	TmpDirectory string `default:"/tmp/phoenixpanel" json:"-" yaml:"tmp_directory"`

	// The user that should own all of the server files, and be used for containers.
	Username string `default:"phoenixpanel" yaml:"username"`
...
type Configuration struct {
	...
	AppName string `default:"phoenixpanel" json:"app_name" yaml:"app_name"`
...
// EnsurephoenixpanelUser ensures that the phoenixpanel core user exists on the
...
func EnsurephoenixpanelUser() error {
	...
	if sysName == "distroless" {
		_config.System.Username = system.FirstNotEmpty(os.Getenv("WINGS_USERNAME"), "phoenixpanel")
	...
	log.WithField("username", _config.System.Username).Info("checking for phoenixpanel system user")
